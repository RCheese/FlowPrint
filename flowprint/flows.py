...
class Flow:
    ...
    def add(self, packet):
        ...
        if self.src is not None:
            if {self.src, self.dst} != {ip_a, ip_b} and {self.sport, self.dport} != {
                port_a,
                port_b,
            }:
                raise ValueError(
                    "Packet {} incompatible with flow {}".format(packet, self)
                )
        ...
        self.lengths.append(
            int(packet[4]) if packet[5] == self.src else -int(packet[4])
        )
    ...
    def __str__(self):
        ...
        return (
            f"[Time {datetime.fromtimestamp(min(self.timestamps)).strftime('%H:%M:%S.%f')} to "
            f"{datetime.fromtimestamp(max(self.timestamps)).strftime('%H:%M:%S.%f')}] "
            f"{self.src:>15}:{self.sport:<5} <-> {self.dst:>15}:{self.dport:<5} [Length {len(self)}]"
        )
