...
class FlowPrint:
    ...
    def __init__(
        self, batch=300, window=30, correlation=0.1, similarity=0.9, threshold=0.1
    ):
        """FlowPrint object for creating fingerprints from mobile network traffic
    ...
    def update(self, X, y=None):
        ...
        if y is None:
            ...
            self.fingerprints = {
                k: v.most_common(1)[0][0] for k, v in self.fingerprints.items()
            }
    ...
    def predict(self, X, y=None, default="common"):
        ...
        for fingerprint in self.fingerprints:
            for dst in fingerprint:
                lookup[dst] = lookup.get(dst, set()) | {fingerprint}
    ...
    def recognize(self, X, y=None, default="common"):
        ...
        return np.asarray(
            [self.fingerprints.get(x) for x in self.predict(X, default=default)]
        )
    ...
    def save(self, file, fingerprints=None):
        ...
        output = {
            ...
            "fingerprints": [
                [fp.to_dict(), self.fingerprints.get(fp, file)]
        ...
        for file in files:
            with open(file, "r") as infile:
                ...
                for fp, label in data.get("fingerprints"):
                    ...
                    # Get label
                    label = fingerprints.get(fp, set()) | {label}
                    # Set fingerprint
